...
export const createGroupHandler = TryCatch(async (req, res, next) => {
    const { name, description } = req.body;
    let createdBy = req.decodedToken.id;
    if (!validateGroupName(name))
        return next(new ErrorHandler('Invalid group name', 400));
    if (description && !validateGroupDescription(description))
        return next(new ErrorHandler('Invalid group description', 400));
    const newGroup = await createGroup({ name, description, createdBy });
    if (newGroup instanceof ErrorHandler)
...
export const addMemberHandler = TryCatch(async (req, res, next) => {
    ...
    if (updatedGroup instanceof ErrorHandler)
        return next(updatedGroup);
    res.status(200).json({
        success: true,
        group: updatedGroup
    });
});
export const removeMemberHandler = TryCatch(async (req, res, next) => {
    const { userIds } = req.body;
    const { groupId } = req.params;
    const requestUserId = req.decodedToken.id;
    if (!isValidObjectId(groupId))
...
export const getGroupHandler = TryCatch(async (req, res, next) => {
    const { id } = req.params;
    if (!isValidObjectId(id))
        return next(new ErrorHandler('Invalid group ID', 400));
    const group = await getGroupById(id);
    if (group instanceof ErrorHandler)
        return next(group);
    res.status(200).json({
        success: true,
        group
...
export const deleteGroupHandler = TryCatch(async (req, res, next) => {
    const { id } = req.params;
...
export const getUserGroupsHandler = TryCatch(async (req, res, next) => {
    const userId = req.decodedToken.id;
